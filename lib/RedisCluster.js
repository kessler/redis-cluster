...
function RedisCluster(serversConfig, redisFactory, keyMapper) {
	...
	for (var i = 0; i < serversConfig.length; i++) {
		var serverEntry = serversConfig[i].host + ':' + serversConfig[i].port;
		this.keyMapper.addServer(serverEntry);		
		var client = redisFactory.createClient(serversConfig[i].port, serversConfig[i].host);
...
function doCommand(redis, command, params, results, callback) {
	return function(callback) {
		...
		if (params.length === 1) {
			redis.send_command(command, params[0], commandCallback(results, params[0][0]));
		} else {
